...
```plain
module Main in execution;%0A%0A@Start%0Adefun main (args : String[]) : void%0A{%0A    val original = new LinkedList([100, 200, 300]);%0A%0A    val copy = F::mutable(original is Iterable);%0A%0A    original.add(400);%0A    copy.add(500);%0A%0A    F::println(original);%0A    F::println(copy);%0A}
```
