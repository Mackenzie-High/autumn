...
```plain
module Main in execution;%0A%0A@Start%0Adefun main (args : String[]) : void%0A{%0A    val original = new HashMap();%0A%0A    original.put(100, 13);%0A    original.put(200, 17);%0A%0A    val copy = F::mutable(original is Map);%0A%0A    original.put(300, 19);%0A    original.put(400, 21);%0A%0A    copy.put(500, 23);%0A    copy.put(600, 27);%0A%0A    F::println(original);%0A    F::println(copy);%0A}
```

**Output:**

```plain
{100=13, 200=17, 400=21, 300=19}%0A{100=13, 200=17, 500=23, 600=27}
```
