# Sequence Statement

## Summary

A sequence-statement executes a series of statements one after another.

## Syntax

<div id="syntax">
{<br>
&nbsp;&nbsp;&nbsp;&nbsp;<i>[statement](TextPage.html?page=Statement)<sub>1</sub></i><br>
&nbsp;&nbsp;&nbsp;&nbsp;<i>[statement](TextPage.html?page=Statement)<sub>2</sub></i><br>
&nbsp;&nbsp;&nbsp;&nbsp;<i>[statement](TextPage.html?page=Statement)<sub>n</sub></i><br>
}<br>
</div>

## AST Class

autumn.lang.compiler.ast.nodes.SequenceStatement

## Details

+ There can be zero or more statements in the sequence.

## Static Checks


## Example

**Code:**

```plain
module Main in examples;

@Start
defun main (args : String[]) : void
{
    {
        F::println("A");
        F::println("B");
        F::println("C");
    }
}
```

**Output:**

```plain
A
B
C
```

