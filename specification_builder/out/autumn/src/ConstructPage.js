...
function emitDetails()
{
    ...
    /**
     * If there are no details to show, then remove the "Details" section.
     * Otherwise, the UI will not look right. 
     */
    if(length == 0)
    {
        $("#construct-page-details").empty();
    }
}
...
function emitStaticChecks()
{
    ...
    /**
     * If there are no stack-checks to show, then remove the "Static Checks" section.
     * Otherwise, the UI will not look right. 
     */
    if(length == 0)
    {
        $("#construct-page-checks").empty();
    }
}
...
function emitAstClass()
{
    ...
    /**
     * If no AST class was specified, then remove the "AST Class" section. 
     * Otherwise, the UI will not look right. 
     */
    if(klass == null)
    {
