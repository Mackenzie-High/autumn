...
/**
 * An instance of this class represents one of the functions in the F class of
 * the Autumn standard library.
 *
 * @author mackenzie
 */
        ...
        implements Comparable<Function>
{
     ...
     *
     * Note; Do not change the case of these constants, because it simplifies
    ...
    /**
     * This method adds a detail that indicates that the function simply invokes
     * a JVM method.
     *
     * @param owner
     * @param name
    ...
    public String id ()
    {
        return Hashing.md5().hashString(toString(), StandardCharsets.US_ASCII).toString();
    }
            ...
            throws IOException
    {
         ...
         * Add the signature.
         */
        markdown.append("## Signature").append("\n");
        markdown.append("\n");
        markdown.append(signature()).append("\n");
        markdown.append("\n");
         ...
         * Add the formals.
         */
        if (formals.isEmpty() == false)
        {
            markdown.append("## Formals").append("\n");
            markdown.append("\n");

            for (Parameter formal : formals)
            {
                markdown.append("+ Parameter <i>").append(formal.name).append("</i> ").append(formal.about).append("\n");
            }
         ...
         * Add the return-type.
         */
        markdown.append("## Returns").append("\n");
        markdown.append("\n");
        markdown.append(returns.about).append("\n");
        markdown.append("\n");
        ...
        {
            markdown.append("## Throws").append("\n");
            markdown.append("\n");

            for (Problem problem : problems)
            {
                markdown.append("+ ").append(problem.toString()).append(", ").append(problem.reason).append("\n");
            }
        ...
        if (details.isEmpty() == false)
        {
            ...
            {
                final var padding = Strings.repeat(" ", 4 * detail.getKey());
                markdown.append("+ ").append(padding).append(detail.getValue()).append("\n");
            }
        ...
        for (Entry<Integer, Integer> example : examples)
        {
            ...
            {
                System.out.println("Bad Example - Function: " + name + ". Example Index: " + example.getValue());
            }
        ...
        final File file = new File(JSONBuilder.FUNCTIONS, this.id() + ".md");
     ...
     *
     * @return the generated Java code that will be placed into the
     * autumn.util.F class.
     */
