package high.mackenzie.autumn.lang.compiler.parser;

import high.mackenzie.snowflake.Grammar;
import high.mackenzie.snowflake.GrammarBuilder;
import high.mackenzie.snowflake.IParser;
import high.mackenzie.snowflake.ParserOutput;


/**
 * This class was auto-generated using the Snowflake parser-generator.
 *
 * <p>Generated On: Mon Apr 21 22:27:40 EDT 2014</p>
 */
public final class Parser implements IParser
{

    private static final Grammar grammar = grammar();


    /**
     * This method constructs the grammar object.
     */
    private static Grammar grammar()
    {
        final GrammarBuilder g = new GrammarBuilder();

        // Character Classes
        g.range("@class0", (char) 64, (char) 64);
        g.range("@class1", (char) 123, (char) 123);
        g.range("@class2", (char) 125, (char) 125);
        g.range("@class3", (char) 58, (char) 58);
        g.range("@class4", (char) 58, (char) 58);
        g.range("@class5", (char) 123, (char) 123);
        g.range("@class6", (char) 125, (char) 125);
        g.range("@class7", (char) 123, (char) 123);
        g.range("@class8", (char) 125, (char) 125);
        g.range("@class9", (char) 40, (char) 40);
        g.range("@class10", (char) 58, (char) 58);
        g.range("@class11", (char) 41, (char) 41);
        g.range("@class12", (char) 40, (char) 40);
        g.range("@class13", (char) 41, (char) 41);
        g.range("@class14", (char) 61, (char) 61);
        g.range("@class15", (char) 40, (char) 40);
        g.range("@class16", (char) 41, (char) 41);
        g.range("@class17", (char) 58, (char) 58);
        g.range("@class18", (char) 61, (char) 61);
        g.range("@class19", (char) 61, (char) 61);
        g.range("@class20", (char) 61, (char) 61);
        g.range("@class21", (char) 46, (char) 46);
        g.range("@class22", (char) 46, (char) 46);
        g.range("@class23", (char) 46, (char) 46);
        g.range("@class24", (char) 38, (char) 38);
        g.range("@class25", (char) 124, (char) 124);
        g.range("@class26", (char) 94, (char) 94);
        g.range("@class27", (char) 62, (char) 62);
        g.range("@class28", (char) 60, (char) 60);
        g.range("@class29", (char) 43, (char) 43);
        g.range("@class30", (char) 45, (char) 45);
        g.range("@class31", (char) 47, (char) 47);
        g.range("@class32", (char) 37, (char) 37);
        g.range("@class33", (char) 42, (char) 42);
        g.range("@class34", (char) 45, (char) 45);
        g.range("@class35", (char) 33, (char) 33);
        g.range("@class36", (char) 40, (char) 40);
        g.range("@class37", (char) 41, (char) 41);
        g.range("@class38", (char) 40, (char) 40);
        g.range("@class39", (char) 41, (char) 41);
        g.range("@class40", (char) 44, (char) 44);
        g.range("@class41", (char) 91, (char) 91);
        g.range("@class42", (char) 93, (char) 93);
        g.range("@class43", (char) 61, (char) 61);
        g.range("@class44", (char) 61, (char) 61);
        g.range("@class45", (char) 58, (char) 58);
        g.range("@class46", (char) 46, (char) 46);
        g.range("@class47", (char) 58, (char) 58);
        g.range("@class48", (char) 40, (char) 40);
        g.range("@class49", (char) 44, (char) 44);
        g.range("@class50", (char) 41, (char) 41);
        g.range("@class51", (char) 58, (char) 58);
        g.range("@class52", (char) 44, (char) 44);
        g.range("@class53", (char) 91, (char) 91);
        g.range("@class54", (char) 93, (char) 93);
        g.range("@class55", (char) 46, (char) 46);
        g.range("@class56", (char) 46, (char) 46);
        g.range("@class57", (char) 0, (char) 65535);
        g.range("@class58", (char) 67, (char) 67);
        g.range("@class59", (char) 45, (char) 45);
        g.range("@class60", (char) 66, (char) 66);
        g.range("@class61", (char) 45, (char) 45);
        g.range("@class62", (char) 83, (char) 83);
        g.range("@class63", (char) 45, (char) 45);
        g.range("@class64", (char) 45, (char) 45);
        g.range("@class65", (char) 76, (char) 76);
        g.range("@class66", (char) 45, (char) 45);
        g.range("@class67", (char) 46, (char) 46);
        g.range("@class68", (char) 70, (char) 70);
        g.range("@class69", (char) 45, (char) 45);
        g.range("@class70", (char) 46, (char) 46);
        g.range("@class71", (char) 40, (char) 40);
        g.range("@class72", (char) 41, (char) 41);
        g.range("@class73", (char) 59, (char) 59);
        g.range("@class74", (char) 65, (char) 90);
        g.range("@class75", (char) 97, (char) 122);
        g.range("@class76", (char) 95, (char) 95);
        g.range("@class77", (char) 36, (char) 36);
        g.combine("@class78", "@class74", "@class75", "@class76", "@class77");
        g.range("@class79", (char) 65, (char) 90);
        g.range("@class80", (char) 97, (char) 122);
        g.range("@class81", (char) 95, (char) 95);
        g.range("@class82", (char) 36, (char) 36);
        g.range("@class83", (char) 48, (char) 57);
        g.combine("@class84", "@class79", "@class80", "@class81", "@class82", "@class83");
        g.range("@class85", (char) 48, (char) 57);
        g.range("@class86", (char) 101, (char) 101);
        g.range("@class87", (char) 69, (char) 69);
        g.combine("@class88", "@class86", "@class87");
        g.range("@class89", (char) 45, (char) 45);
        g.range("@class90", (char) 48, (char) 57);
        g.combine("@class91", "@class89", "@class90");
        g.range("@class92", (char) 48, (char) 57);
        g.range("@class93", (char) 34, (char) 34);
        g.range("@class94", (char) 34, (char) 34);
        g.combine("@class95", "@class94");
        g.negate("@class96", "@class95");
        g.range("@class97", (char) 39, (char) 39);
        g.combine("@class98", "@class97");
        g.negate("@class99", "@class98");
        g.range("@class100", (char) 32, (char) 32);
        g.range("@class101", (char) 9, (char) 9);
        g.range("@class102", (char) 10, (char) 10);
        g.range("@class103", (char) 11, (char) 11);
        g.range("@class104", (char) 12, (char) 12);
        g.range("@class105", (char) 13, (char) 13);
        g.combine("@class106", "@class100", "@class101", "@class102", "@class103", "@class104", "@class105");
        g.range("@class107", (char) 35, (char) 35);
        g.range("@class108", (char) 10, (char) 10);
        g.range("@class109", (char) 13, (char) 13);
        g.combine("@class110", "@class108", "@class109");
        g.negate("@class111", "@class110");
        g.range("@class112", (char) 10, (char) 10);
        g.range("@class113", (char) 13, (char) 13);
        g.combine("@class114", "@class112", "@class113");
        g.range("@class115", (char) 0, (char) 65535);
        g.combine("@class116", "@class115");
        g.range("@class117", (char) 10, (char) 10);
        g.range("@class118", (char) 13, (char) 13);
        g.combine("@class119", "@class117", "@class118");
        g.range("@class120", (char) 10, (char) 10);
        g.range("@class121", (char) 13, (char) 13);
        g.combine("@class122", "@class120", "@class121");
        g.negate("@class123", "@class122");

        // Grammar Rules
        g.choose("@209", "@208", "COMMENT");
        g.choose("@212", "@210", "@211");
        g.choose("@56", "do_until", "do_while");
        g.choose("COMMENT", "MULTI_LINE_COMMENT", "SINGLE_LINE_COMMENT");
        g.choose("NON_TRIPLE_QUOTE", "@206", "@207");
        g.choose("STRING_LITERAL", "STRING_LITERAL_STYLE1", "STRING_LITERAL_STYLE2");
        g.choose("assertion_echo_opt", "assertion_echo", "assertion_echo_empty");
        g.choose("boolean_value", "true", "false");
        g.choose("char_value", "literal_char_value", "numeric_char_value");
        g.choose("datum", "string_datum", "boolean_datum", "char_datum", "float_datum", "double_datum", "long_datum", "short_datum", "byte_datum", "int_datum", "null_datum", "class_datum", "variable_datum");
        g.choose("definition", "function_definition", "exception_definition", "design_definition", "enum_definition", "annotation_definition");
        g.choose("design_extends_opt", "design_extends", "design_extends_empty");
        g.choose("design_member", "property", "method");
        g.choose("expression", "p1", "keyword_expression", "nested_expression");
        g.choose("extends_list", "extends_list_cons", "extends_list_end");
        g.choose("keyword_expression_body", "funcall_expression", "delegate_expression_simplified", "locals_expression", "dispatch_expression", "new_expression", "call_static_method_expression", "call_method_expression", "set_static_expression", "get_static_expression", "set_field_expression", "get_field_expression", "instanceof_expression", "create_expression", "ternary_conditional_expression", "progn_expression");
        g.choose("module_member", "include_directive", "use_directive", "module_directive", "import_directive", "definition");
        g.choose("module_name", "name_of_anonymous_module", "name_of_named_module");
        g.choose("namespace", "namespace_explicit", "namespace_implicit");
        g.choose("p1_operation", "as_operation", "is_operation");
        g.choose("p3_operation", "short_circuit_and_operation", "short_circuit_or_operation", "and_operation", "or_operation", "xor_operation");
        g.choose("p4_operation", "identity_equals_operation", "identity_not_equals_operation", "equals_operation", "not_equals_operation", "greater_or_equals_operation", "less_or_equals_operation", "greater_operation", "less_operation");
        g.choose("p6_operation", "add_operation", "subtract_operation");
        g.choose("p7_operation", "divide_operation", "modulo_operation", "multiply_operation");
        g.choose("p8", "datum", "negate_operation", "not_operation", "keyword_expression", "nested_expression", "list_expression", "delegate_expression");
        g.choose("statement", "sequence_statement", "if_statement", "when_statement", "goto_statement", "marker_statement", "do_statement", "break_statement", "continue_statement", "redo_statement", "foreach_statement", "for_statement", "until_statement", "while_statement", "assert_statement", "throw_statement", "try_catch_statement", "var_statement", "val_statement", "let_statement", "setter_statement", "getter_statement", "method_statement", "nop_statement", "return_value_statement", "return_void_statement", "recur_statement", "yield_value_statement", "yield_void_statement", "expression_statement");
        g.choose("type_namespace", "type_namespace_explicit", "type_namespace_implicit");
        g.chr("@100", "@class32");
        g.chr("@101", "@class33");
        g.chr("@102", "@class34");
        g.chr("@103", "@class35");
        g.chr("@104", "@class36");
        g.chr("@105", "@class37");
        g.chr("@106", "@class38");
        g.chr("@107", "@class39");
        g.chr("@11", "@class1");
        g.chr("@111", "@class40");
        g.chr("@12", "@class2");
        g.chr("@120", "@class41");
        g.chr("@121", "@class42");
        g.chr("@131", "@class43");
        g.chr("@134", "@class44");
        g.chr("@136", "@class45");
        g.chr("@139", "@class46");
        g.chr("@143", "@class47");
        g.chr("@144", "@class48");
        g.chr("@145", "@class49");
        g.chr("@150", "@class50");
        g.chr("@151", "@class51");
        g.chr("@152", "@class52");
        g.chr("@157", "@class53");
        g.chr("@158", "@class54");
        g.chr("@159", "@class55");
        g.chr("@16", "@class3");
        g.chr("@162", "@class56");
        g.chr("@169", "@class57");
        g.chr("@171", "@class58");
        g.chr("@172", "@class59");
        g.chr("@174", "@class60");
        g.chr("@175", "@class61");
        g.chr("@177", "@class62");
        g.chr("@178", "@class63");
        g.chr("@18", "@class4");
        g.chr("@180", "@class64");
        g.chr("@182", "@class65");
        g.chr("@183", "@class66");
        g.chr("@185", "@class67");
        g.chr("@186", "@class68");
        g.chr("@187", "@class69");
        g.chr("@189", "@class70");
        g.chr("@190", "@class71");
        g.chr("@192", "@class72");
        g.chr("@194", "@class73");
        g.chr("@195", "@class78");
        g.chr("@196", "@class84");
        g.chr("@198", "@class85");
        g.chr("@199", "@class88");
        g.chr("@20", "@class5");
        g.chr("@200", "@class91");
        g.chr("@201", "@class92");
        g.chr("@207", "@class99");
        g.chr("@208", "@class106");
        g.chr("@21", "@class6");
        g.chr("@211", "@class107");
        g.chr("@213", "@class111");
        g.chr("@215", "@class114");
        g.chr("@219", "@class116");
        g.chr("@24", "@class7");
        g.chr("@25", "@class8");
        g.chr("@38", "@class9");
        g.chr("@39", "@class10");
        g.chr("@40", "@class11");
        g.chr("@45", "@class12");
        g.chr("@46", "@class13");
        g.chr("@47", "@class14");
        g.chr("@49", "@class15");
        g.chr("@51", "@class16");
        g.chr("@52", "@class17");
        g.chr("@6", "@class0");
        g.chr("@60", "@class18");
        g.chr("@62", "@class19");
        g.chr("@65", "@class20");
        g.chr("@67", "@class21");
        g.chr("@70", "@class22");
        g.chr("@73", "@class23");
        g.chr("@85", "@class24");
        g.chr("@86", "@class25");
        g.chr("@87", "@class26");
        g.chr("@94", "@class27");
        g.chr("@95", "@class28");
        g.chr("@97", "@class29");
        g.chr("@98", "@class30");
        g.chr("@99", "@class31");
        g.chr("NEWLINE", "@class119");
        g.chr("NON_NEWLINE", "@class123");
        g.chr("NON_QUOTE", "@class96");
        g.chr("QUOTE", "@class93");
        g.not("@109", "ID");
        g.not("@165", "ID");
        g.not("@167", "ID");
        g.not("@205", "@204");
        g.not("@218", "@217");
        g.repeat("@113", "@112", 0, 1);
        g.repeat("@126", "@125", 0, 1);
        g.repeat("@128", "@127", 0, 1);
        g.repeat("@141", "@140", 0, 2147483647);
        g.repeat("@147", "@146", 0, 2147483647);
        g.repeat("@149", "@148", 0, 1);
        g.repeat("@154", "@153", 0, 2147483647);
        g.repeat("@161", "@160", 1, 2147483647);
        g.repeat("@173", "@172", 0, 1);
        g.repeat("@176", "@175", 0, 1);
        g.repeat("@179", "@178", 0, 1);
        g.repeat("@181", "@180", 0, 1);
        g.repeat("@184", "@183", 0, 1);
        g.repeat("@188", "@187", 0, 1);
        g.repeat("@197", "@196", 0, 2147483647);
        g.repeat("@202", "@201", 0, 2147483647);
        g.repeat("@214", "@213", 0, 2147483647);
        g.repeat("@221", "@220", 0, 2147483647);
        g.repeat("@5", "annotation", 0, 2147483647);
        g.repeat("@64", "@63", 0, 1);
        g.repeat("DIGITS", "@198", 1, 2147483647);
        g.repeat("EXPONENT_OPT", "EXPONENT", 0, 1);
        g.repeat("NON_NEWLINES", "NON_NEWLINE", 0, 2147483647);
        g.repeat("NON_QUOTES", "NON_QUOTE", 0, 2147483647);
        g.repeat("NON_TRIPLE_QUOTES", "NON_TRIPLE_QUOTE", 0, 2147483647);
        g.repeat("WS", "@209", 0, 2147483647);
        g.repeat("arguments", "@155", 0, 1);
        g.repeat("design_members", "design_member", 0, 2147483647);
        g.repeat("dimensions", "@156", 0, 2147483647);
        g.repeat("elif_cases_opt", "elif_case", 0, 2147483647);
        g.repeat("else_case_opt", "else_case", 0, 1);
        g.repeat("enum_constants", "enum_constant", 0, 2147483647);
        g.repeat("module_members", "module_member", 0, 2147483647);
        g.repeat("p1_operations", "p1_operation", 0, 2147483647);
        g.repeat("p2_operations", "p2_operation", 0, 2147483647);
        g.repeat("p3_operations", "p3_operation", 0, 2147483647);
        g.repeat("p4_operations", "p4_operation", 0, 2147483647);
        g.repeat("p5_operations", "p5_operation", 0, 2147483647);
        g.repeat("p6_operations", "p6_operation", 0, 2147483647);
        g.repeat("p7_operations", "p7_operation", 0, 2147483647);
        g.repeat("statements", "statement", 0, 2147483647);
        g.repeat("trycatch_handlers", "trycatch_handler", 0, 2147483647);
        g.repeat("wraps_opt", "wraps", 0, 1);
        g.sequence("@112", "WS", "@111", "WS", "arguments");
        g.sequence("@140", "@139", "name_id");
        g.sequence("@142", "name_id", "@141");
        g.sequence("@146", "WS", "@145", "WS", "parameter");
        g.sequence("@148", "parameter", "@147");
        g.sequence("@153", "WS", "@152", "WS", "WS", "argument");
        g.sequence("@155", "argument", "@154", "WS");
        g.sequence("@156", "dimension", "WS");
        g.sequence("@160", "name_id", "@159");
        g.sequence("@206", "@203", "@205");
        g.sequence("@220", "@218", "@219");
        g.sequence("EXPONENT", "@199", "@200", "@202");
        g.sequence("ID", "@195", "@197");
        g.sequence("MULTI_LINE_COMMENT", "@216", "@221", "@222", "WS");
        g.sequence("SEMICOLON", "@194", "WS");
        g.sequence("SINGLE_LINE_COMMENT", "@212", "@214", "@215");
        g.sequence("STRING_LITERAL_STYLE1", "QUOTE", "NON_QUOTES", "QUOTE");
        g.sequence("STRING_LITERAL_STYLE2", "TRIPLE_QUOTE", "NON_TRIPLE_QUOTES", "TRIPLE_QUOTE");
        g.sequence("add_operation", "@97", "WS", "p6_operand");
        g.sequence("and_operation", "@85", "WS", "p3_operand");
        g.sequence("annotation", "PS", "@6", "WS", "annotation_type", "WS");
        g.sequence("annotation_definition", "PS", "annotation_list", "WS", "@7", "WS", "name", "WS", "SEMICOLON", "WS");
        g.sequence("annotation_list", "PS", "@5");
        g.sequence("annotation_type", "type");
        g.sequence("argument", "expression");
        g.sequence("as_operation", "@80", "WS", "type", "WS");
        g.sequence("assert_statement", "PS", "@33", "WS", "condition", "WS", "assertion_echo_opt", "WS", "SEMICOLON");
        g.sequence("assertion_echo", "@34", "WS", "expression", "WS");
        g.sequence("boolean_datum", "boolean_value");
        g.sequence("break_statement", "PS", "@41", "WS", "SEMICOLON");
        g.sequence("byte_datum", "byte_value");
        g.sequence("byte_value", "@173", "DIGITS", "@174", "WS");
        g.sequence("call_method_expression", "PS", "@126", "WS", "instance_member_access", "arguments", "WS");
        g.sequence("call_static_method_expression", "PS", "@128", "WS", "static_member_access", "arguments", "WS");
        g.sequence("char_datum", "char_value");
        g.sequence("class_datum", "class_value", "WS");
        g.sequence("class_value", "@190", "WS", "@191", "WS", "type", "WS", "@192", "WS");
        g.sequence("concat_operation", "@96", "WS", "p5_operand");
        g.sequence("condition", "expression");
        g.sequence("conditional_case", "PS", "paren_condition", "WS", "sequence_statement", "WS");
        g.sequence("continue_statement", "PS", "@42", "WS", "SEMICOLON");
        g.sequence("create_expression", "PS", "@137", "WS", "type", "WS", "wraps_opt", "WS");
        g.sequence("delegate_expression", "PS", "formals", "WS", "@114", "WS", "static_member_access", "WS");
        g.sequence("delegate_expression_simplified", "PS", "@115", "WS", "static_member_access", "WS");
        g.sequence("design_definition", "PS", "annotation_list", "WS", "@10", "WS", "name", "WS", "design_extends_opt", "WS", "@11", "WS", "design_members", "@12", "WS");
        g.sequence("design_extends", "@13", "WS", "extends_list");
        g.sequence("dimension", "@157", "WS", "@158");
        g.sequence("dispatch_expression", "PS", "@123", "WS", "name", "WS", "arguments", "WS");
        g.sequence("divide_operation", "@99", "WS", "p7_operand");
        g.sequence("do_statement", "PS", "@55", "WS", "sequence_statement", "WS", "@56", "WS");
        g.sequence("do_until", "@57", "WS", "paren_condition", "WS", "do_until_trigger");
        g.sequence("do_while", "@58", "WS", "paren_condition", "WS", "do_while_trigger");
        g.sequence("double_datum", "double_value");
        g.sequence("double_value", "@188", "DIGITS", "@189", "DIGITS", "EXPONENT_OPT", "WS");
        g.sequence("elif_case", "@28", "WS", "conditional_case", "WS");
        g.sequence("else_case", "@29", "WS", "sequence_statement", "WS");
        g.sequence("enum_constant", "PS", "annotation_list", "WS", "@22", "WS", "name", "WS", "SEMICOLON", "WS");
        g.sequence("enum_definition", "PS", "annotation_list", "WS", "@19", "WS", "name", "WS", "@20", "WS", "enum_constants", "WS", "@21", "WS");
        g.sequence("enum_value", "@193", "WS", "static_member_access", "WS");
        g.sequence("equals_operation", "@90", "WS", "p4_operand");
        g.sequence("exception_definition", "PS", "annotation_list", "WS", "@8", "WS", "name", "WS", "@9", "WS", "type", "WS", "SEMICOLON", "WS");
        g.sequence("expression_statement", "PS", "expression", "SEMICOLON");
        g.sequence("extends_list_cons", "type", "WS", "@14", "WS", "extends_list");
        g.sequence("extends_list_end", "type");
        g.sequence("false", "@166", "@167", "WS");
        g.sequence("filepath", "string_value");
        g.sequence("float_datum", "float_value");
        g.sequence("float_value", "@184", "DIGITS", "@185", "DIGITS", "EXPONENT_OPT", "@186", "WS");
        g.sequence("for_body", "sequence_statement");
        g.sequence("for_condition", "expression", "WS");
        g.sequence("for_controller", "@45", "WS", "for_init", "WS", "SEMICOLON", "WS", "for_condition", "WS", "SEMICOLON", "WS", "for_modifier", "WS", "@46", "WS");
        g.sequence("for_init", "variable", "WS", "@47", "WS", "expression", "WS");
        g.sequence("for_modifier", "expression");
        g.sequence("for_statement", "PS", "@44", "WS", "for_controller", "WS", "for_body", "WS");
        g.sequence("foreach_body", "sequence_statement");
        g.sequence("foreach_controller", "@49", "WS", "foreach_vardec", "@50", "WS", "foreach_iterator", "@51");
        g.sequence("foreach_iterator", "expression", "WS");
        g.sequence("foreach_statement", "PS", "@48", "WS", "foreach_controller", "WS", "foreach_body", "WS");
        g.sequence("foreach_vardec", "variable", "WS", "@52", "WS", "type", "WS");
        g.sequence("formals", "PS", "@144", "WS", "@149", "WS", "@150", "WS");
        g.sequence("funcall_expression", "PS", "@110", "WS", "expression", "@113", "WS");
        g.sequence("function_definition", "PS", "annotation_list", "WS", "@23", "WS", "signature", "WS", "sequence_statement", "WS");
        g.sequence("get_field_expression", "PS", "@129", "WS", "instance_member_access", "WS");
        g.sequence("get_static_expression", "PS", "@132", "WS", "static_member_access", "WS");
        g.sequence("getter_statement", "PS", "@69", "WS", "variable", "WS", "@70", "WS", "name", "WS", "@71", "WS", "static_member_access", "WS", "SEMICOLON");
        g.sequence("goto_statement", "PS", "@31", "WS", "label", "WS", "SEMICOLON");
        g.sequence("greater_operation", "@94", "WS", "p4_operand");
        g.sequence("greater_or_equals_operation", "@92", "WS", "p4_operand");
        g.sequence("identity_equals_operation", "@88", "WS", "p4_operand");
        g.sequence("identity_not_equals_operation", "@89", "WS", "p4_operand");
        g.sequence("if_case", "@27", "WS", "conditional_case", "WS");
        g.sequence("if_statement", "PS", "if_case", "elif_cases_opt", "WS", "else_case_opt", "WS");
        g.sequence("import_directive", "PS", "@4", "WS", "type", "WS", "SEMICOLON");
        g.sequence("include_directive", "PS", "@1", "WS", "filepath", "WS", "SEMICOLON");
        g.sequence("instance_member_access", "expression", "WS", "@162", "WS", "name", "WS");
        g.sequence("instanceof_expression", "PS", "@135", "WS", "expression", "WS", "@136", "WS", "type", "WS");
        g.sequence("int_datum", "int_value");
        g.sequence("int_value", "@179", "DIGITS", "WS");
        g.sequence("is_operation", "@81", "WS", "type", "WS");
        g.sequence("keyword_expression", "@104", "WS", "keyword_expression_body", "WS", "@105", "WS");
        g.sequence("label", "ID", "WS");
        g.sequence("less_operation", "@95", "WS", "p4_operand");
        g.sequence("less_or_equals_operation", "@93", "WS", "p4_operand");
        g.sequence("let_statement", "PS", "@64", "WS", "variable", "WS", "@65", "WS", "expression", "WS", "SEMICOLON");
        g.sequence("list_expression", "PS", "@120", "WS", "arguments", "WS", "@121", "WS");
        g.sequence("literal_char_value", "@168", "@169", "@170", "WS");
        g.sequence("locals_expression", "PS", "@116", "WS");
        g.sequence("long_datum", "long_value");
        g.sequence("long_value", "@181", "DIGITS", "@182", "WS");
        g.sequence("marker_statement", "PS", "@32", "WS", "label", "WS", "SEMICOLON");
        g.sequence("method", "PS", "annotation_list", "WS", "@17", "WS", "name", "WS", "formals", "WS", "@18", "WS", "type", "WS", "SEMICOLON", "WS");
        g.sequence("method_statement", "PS", "@72", "WS", "variable", "WS", "@73", "WS", "name", "WS", "formals", "WS", "@74", "WS", "static_member_access", "WS", "SEMICOLON");
        g.sequence("module", "WS", "module_members", "END");
        g.sequence("module_directive", "PS", "annotation_list", "WS", "@2", "WS", "module_name", "WS", "@3", "WS", "namespace", "WS", "SEMICOLON");
        g.sequence("modulo_operation", "@100", "WS", "p7_operand");
        g.sequence("multiply_operation", "@101", "WS", "p7_operand");
        g.sequence("name", "name_id", "WS");
        g.sequence("name_id", "ID");
        g.sequence("name_of_named_module", "name");
        g.sequence("namespace_explicit", "PS", "@142");
        g.sequence("negate_operation", "@102", "WS", "p8", "WS");
        g.sequence("nested_expression", "@106", "WS", "expression", "WS", "@107", "WS");
        g.sequence("new_expression", "PS", "@124", "WS", "type", "WS", "arguments", "WS");
        g.sequence("nop_statement", "PS", "@26", "SEMICOLON");
        g.sequence("not_equals_operation", "@91", "WS", "p4_operand");
        g.sequence("not_operation", "@103", "WS", "p8", "WS");
        g.sequence("null_coalescing_operation", "@82", "WS", "p2_operand");
        g.sequence("null_datum", "@108", "@109", "WS");
        g.sequence("numeric_char_value", "DIGITS", "@171", "WS");
        g.sequence("or_operation", "@86", "WS", "p3_operand");
        g.sequence("p1", "p1_operand", "p1_operations");
        g.sequence("p1_operand", "p2");
        g.sequence("p2", "p2_operand", "p2_operations");
        g.sequence("p2_operand", "p3");
        g.sequence("p2_operation", "null_coalescing_operation");
        g.sequence("p3", "p3_operand", "p3_operations");
        g.sequence("p3_operand", "p4");
        g.sequence("p4", "p4_operand", "p4_operations");
        g.sequence("p4_operand", "p5");
        g.sequence("p5", "p5_operand", "p5_operations");
        g.sequence("p5_operand", "p6");
        g.sequence("p5_operation", "concat_operation");
        g.sequence("p6", "p6_operand", "p6_operations");
        g.sequence("p6_operand", "p7");
        g.sequence("p7", "p7_operand", "p7_operations");
        g.sequence("p7_operand", "p8");
        g.sequence("parameter", "PS", "variable", "WS", "@151", "WS", "type", "WS");
        g.sequence("paren_condition", "nested_expression");
        g.sequence("paren_expression", "nested_expression");
        g.sequence("progn_expression", "PS", "@122", "WS", "arguments", "WS");
        g.sequence("property", "PS", "annotation_list", "WS", "@15", "WS", "name", "WS", "@16", "WS", "type", "WS", "SEMICOLON", "WS");
        g.sequence("recur_statement", "PS", "@75", "WS", "arguments", "WS", "SEMICOLON");
        g.sequence("redo_statement", "PS", "@43", "WS", "SEMICOLON");
        g.sequence("return_type", "type");
        g.sequence("return_value_statement", "PS", "@77", "WS", "expression", "WS", "SEMICOLON");
        g.sequence("return_void_statement", "PS", "@76", "WS", "SEMICOLON");
        g.sequence("sequence_statement", "PS", "@24", "WS", "statements", "WS", "@25", "WS");
        g.sequence("set_field_expression", "PS", "@130", "WS", "instance_member_access", "@131", "WS", "expression", "WS");
        g.sequence("set_static_expression", "PS", "@133", "WS", "static_member_access", "WS", "@134", "WS", "expression", "WS");
        g.sequence("setter_statement", "PS", "@66", "WS", "variable", "WS", "@67", "WS", "name", "WS", "@68", "WS", "static_member_access", "WS", "SEMICOLON");
        g.sequence("short_circuit_and_operation", "@83", "WS", "p3_operand");
        g.sequence("short_circuit_or_operation", "@84", "WS", "p3_operand");
        g.sequence("short_datum", "short_value");
        g.sequence("short_value", "@176", "DIGITS", "@177", "WS");
        g.sequence("signature", "name", "WS", "formals", "WS", "@143", "WS", "return_type", "WS");
        g.sequence("simple_name", "name_id");
        g.sequence("static_member_access", "static_owner", "WS", "name", "WS");
        g.sequence("static_owner", "static_owner_explicit");
        g.sequence("static_owner_explicit", "type", "WS", "@163");
        g.sequence("string_datum", "string_value");
        g.sequence("string_value", "STRING_LITERAL", "WS");
        g.sequence("subtract_operation", "@98", "WS", "p6_operand");
        g.sequence("ternary_conditional_expression", "PS", "@117", "WS", "condition", "WS", "@118", "WS", "expression", "@119", "WS", "expression", "WS");
        g.sequence("throw_statement", "PS", "@35", "WS", "expression", "WS", "SEMICOLON");
        g.sequence("true", "@164", "@165", "WS");
        g.sequence("try_catch_statement", "PS", "@36", "WS", "trycatch_body", "WS", "trycatch_handlers", "WS");
        g.sequence("trycatch_body", "sequence_statement");
        g.sequence("trycatch_handler", "PS", "@37", "WS", "trycatch_vardec", "WS", "sequence_statement", "WS");
        g.sequence("trycatch_vardec", "@38", "WS", "variable", "WS", "@39", "WS", "type", "WS", "@40");
        g.sequence("type", "type_name", "WS", "dimensions", "WS");
        g.sequence("type_name", "type_namespace", "simple_name", "WS");
        g.sequence("type_namespace_explicit", "PS", "@161");
        g.sequence("until_statement", "PS", "@53", "WS", "paren_condition", "WS", "sequence_statement", "WS");
        g.sequence("use_directive", "PS", "@0", "WS", "filepath", "WS", "SEMICOLON");
        g.sequence("val_statement", "PS", "@61", "WS", "variable", "WS", "@62", "WS", "expression", "WS", "SEMICOLON");
        g.sequence("var_statement", "PS", "@59", "WS", "variable", "WS", "@60", "WS", "expression", "WS", "SEMICOLON");
        g.sequence("variable", "ID", "WS");
        g.sequence("variable_datum", "variable");
        g.sequence("when_statement", "PS", "@30", "WS", "paren_condition", "WS", "statement", "WS");
        g.sequence("while_statement", "PS", "@54", "WS", "paren_condition", "WS", "sequence_statement", "WS");
        g.sequence("wraps", "@138", "WS", "expression", "WS");
        g.sequence("xor_operation", "@87", "WS", "p3_operand");
        g.sequence("yield_value_statement", "PS", "@79", "WS", "expression", "WS", "SEMICOLON");
        g.sequence("yield_void_statement", "PS", "@78", "WS", "SEMICOLON");
        g.str("@0", "use");
        g.str("@1", "include");
        g.str("@10", "design");
        g.str("@108", "null");
        g.str("@110", "funcall");
        g.str("@114", "=>");
        g.str("@115", "delegate");
        g.str("@116", "locals");
        g.str("@117", "if");
        g.str("@118", "then");
        g.str("@119", "else");
        g.str("@122", "progn");
        g.str("@123", "dispatch");
        g.str("@124", "new");
        g.str("@125", "call");
        g.str("@127", "call");
        g.str("@129", "field");
        g.str("@13", "extends");
        g.str("@130", "field");
        g.str("@132", "field");
        g.str("@133", "field");
        g.str("@135", "instanceof");
        g.str("@137", "create");
        g.str("@138", "wraps");
        g.str("@14", "&");
        g.str("@15", "data");
        g.str("@163", "::");
        g.str("@164", "true");
        g.str("@166", "false");
        g.str("@168", "'");
        g.str("@17", "method");
        g.str("@170", "'");
        g.str("@19", "enum");
        g.str("@191", "class");
        g.str("@193", "enum");
        g.str("@2", "module");
        g.str("@203", "'");
        g.str("@204", "''");
        g.str("@210", "//");
        g.str("@216", "/*");
        g.str("@217", "*/");
        g.str("@22", "constant");
        g.str("@222", "*/");
        g.str("@23", "defun");
        g.str("@26", "nop");
        g.str("@27", "if");
        g.str("@28", "elif");
        g.str("@29", "else");
        g.str("@3", "in");
        g.str("@30", "when");
        g.str("@31", "goto");
        g.str("@32", "marker");
        g.str("@33", "assert");
        g.str("@34", "echo");
        g.str("@35", "throw");
        g.str("@36", "try");
        g.str("@37", "catch");
        g.str("@4", "import");
        g.str("@41", "break");
        g.str("@42", "continue");
        g.str("@43", "redo");
        g.str("@44", "for");
        g.str("@48", "foreach");
        g.str("@50", "in");
        g.str("@53", "until");
        g.str("@54", "while");
        g.str("@55", "do");
        g.str("@57", "until");
        g.str("@58", "while");
        g.str("@59", "var");
        g.str("@61", "val");
        g.str("@63", "let");
        g.str("@66", "setter");
        g.str("@68", "=>");
        g.str("@69", "getter");
        g.str("@7", "annotation");
        g.str("@71", "=>");
        g.str("@72", "method");
        g.str("@74", "=>");
        g.str("@75", "recur");
        g.str("@76", "return");
        g.str("@77", "return");
        g.str("@78", "yield");
        g.str("@79", "yield");
        g.str("@8", "exception");
        g.str("@80", "as");
        g.str("@81", "is");
        g.str("@82", "??");
        g.str("@83", "&&");
        g.str("@84", "||");
        g.str("@88", "===");
        g.str("@89", "!==");
        g.str("@9", "extends");
        g.str("@90", "==");
        g.str("@91", "!=");
        g.str("@92", ">=");
        g.str("@93", "<=");
        g.str("@96", "..");
        g.str("PS", "");
        g.str("TRIPLE_QUOTE", "'''");
        g.str("assertion_echo_empty", "");
        g.str("design_extends_empty", "");
        g.str("do_until_trigger", "");
        g.str("do_while_trigger", "");
        g.str("name_of_anonymous_module", "*");
        g.str("namespace_implicit", "");
        g.str("static_owner_implicit", "");
        g.str("type_namespace_implicit", "");

        // Specify which rule is the root of the grammar.
        g.setRoot("module");

        // Specify the number of tracing records to store concurrently.
        g.setTraceCount(1024);

        // Perform the actual construction of the grammar object.
        return g.build();
    }


    /**
     * {@inheritDoc}
     */
    @Override
    public ParserOutput parse(final char[] input)
    {
        return grammar.newParser().parse(input);
    }


    /**
     * {@inheritDoc}
     */
    @Override
    public ParserOutput parse(final String input)
    {
        return parse(input.toCharArray());
    }
}
