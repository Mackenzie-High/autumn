...
//        method.instructions.add(new InsnNode(Opcodes.RETURN));
    ...
    private void addReentryTable()
    {
        ...
        //
        // GETSTATIC yield-field          - Get the object that stores the generator's state.
        // INVOKEVIRTUAL getIndex()       - Get the reentry index from the state.
        //
        // GETSTATIC yield-field          - Get the object that stores the generator's state.
        // LDC -1                         - Load negative one onto the operand-stack.
        // INVOKEVIRTUAL setIndex(int)    - Set the reentry index to negative one.
        //                                - In other words, clear the reentry index.
        //                                - This is needed in case an exception is thrown.
        //                                - Otherwise, an exception would mess up the next reentry.
        //
        // TABLESWITCH yield-points       - Jump to the yield statement that yielded last.
        //                                - No jump will occur, if no yield has yielded.
        //
        //
        ///////////////////////////////////////////////////////////////////////////////////////////
        ...
        // Clear the jump index in case an exception is thrown during the invocation.
        owner = Utils.internalName(program.typesystem.utils.YIELD_STATE);
        name = "setIndex";
        desc = "(I)V";
        ...
        final LabelNode[] label_array = (LabelNode[]) yields.toArray(new LabelNode[0]);
        final TableSwitchInsnNode table = new TableSwitchInsnNode(min, max, END, label_array);
    ...
    private void addRestorationCode()
    {
        ...
        /**
         * If the function does not contain a yield statement,
         * then there is no need for restoration.
         */
        if (yield_field == null)
        {
            return;
